...
if not exist "%ISCC%" (
  ...
  echo Installez Inno Setup 6 pour generer l'installeur, ou distribuez dist\AnonymiseurCR.exe tel quel.
  pause & exit /b 0
)
"%ISCC%" installer.iss
echo.
echo Termine :
echo   - Application autonome : dist\AnonymiseurCR.exe
echo   - Installeur           : installer\AnonymiseurCR_Setup_1.0.0.exe
pause
